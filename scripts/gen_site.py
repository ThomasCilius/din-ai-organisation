...
INSTALL_URL = f"https://{DOMAIN}/install.sh"
LOCAL_CMD  = "~/.claude/din-ai-org/install.sh"
BRAND      = "Din AI Assistent - ThomasCilius.dk"

# Prompterne naevner med vilje IKKE github.com. Peger man Claude paa en
# GitHub-URL, griber den efter git eller GitHub-connectoren - og saa staar
# kursisten pludselig med et login-spoergsmaal midt i en installation, der
# ikke kraever konto overhovedet. Alt hentes over almindelig https fra sitet.
CC_ONELINER = (
  f"Installer {BRAND} for mig. Alt hentes over almindelig https fra {DOMAIN}: "
  "brug IKKE git, gh eller GitHub-connectoren, og bed mig aldrig om at logge ind nogen steder. "
  f"1) Hent installeren: curl -fsSL {INSTALL_URL} -o ~/Downloads/din-ai-assistent.sh   "
  "2) Vis mig planen FOERST: koer 'bash ~/Downloads/din-ai-assistent.sh plan', gengiv de fem trin "
  "med cirka tidsforbrug, og spoerg om jeg er klar.   "
  "3) Naar jeg siger ja: 'bash ~/Downloads/din-ai-assistent.sh install'.   "
  "4) Spoerg derefter, hvor min company-brain ligger (eller skal ligge), og koer "
  "'bash ~/Downloads/din-ai-assistent.sh brain <sti>' - uden den kobling indlaeses hjernen aldrig ambient.   "
  f"5) Vis mig til sidst '{LOCAL_CMD} status' og forklar hver linje, der ikke er [OK]."
)
UPDATE_ONELINER = (
  f"Opdater min {BRAND}: koer '{LOCAL_CMD} update'. Den henter nyeste version over almindelig "
  f"https fra {DOMAIN} - ingen git, ingen GitHub-konto, intet login, og den roerer ikke mine egne "
  f"filer. Vis mig bagefter '{LOCAL_CMD} status'."
)
# Hurtigst i terminalen: ét kald, ingen filer at holde styr paa.
GIT_BLOCK = f"curl -fsSL {INSTALL_URL} | bash"
INSTALLER_BLOCK = (
  f"curl -fsSL {INSTALL_URL} -o din-ai-assistent.sh   # hent installeren\n"
  "bash din-ai-assistent.sh plan          # de fem trin med cirka tidsforbrug\n"
  "bash din-ai-assistent.sh install       # installer hele pakken\n"
  "bash din-ai-assistent.sh brain <sti>   # kobl din company-brain\n"
  "\n"
  f"{LOCAL_CMD} status      # sundhedstjek naar som helst\n"
  f"{LOCAL_CMD} update      # hent nyeste version\n"
  f"{LOCAL_CMD} uninstall   # fjern kun det, installeren lagde"
)

# Overblikket over hele forloebet. Samme fem trin og samme tidsangivelser som
# './install.sh plan' skriver i Claude - ét sted at rette, to steder det vises.
PLAN = [
  ("Vælg hjernens mappe",
   "En helt almindelig, synlig mappe du selv vælger - fx <span class=\"inline\">Dokumenter/company-brain</span>.",
   "ca. 1 min"),
  ("Byg din company brain",
   "Claude interviewer dig og bygger hjernen fase for fase. Medbring 3-5 dokumenter: gamle tilbud, prisliste, om os-tekst.",
   "ca. 20-30 min"),
  ("Installér skills-pakken",
   "Én kommando. Skills, hooks og hjerne-kobling falder på plads af sig selv - ingen konto, intet login.",
   "ca. 2 min"),
  ("Udfyld hub-filerne",
   "Tre skills, én ad gangen: <span class=\"inline\">virksomhedsprofil</span>, <span class=\"inline\">toneprofil</span>, <span class=\"inline\">designretning</span>.",
   "ca. 15-25 min"),
  ("Tjek at det hele hænger sammen",
   "Sundhedstjekket svarer [OK] eller [MANGLER] linje for linje - og hvad du gør ved hver linje.",
   "ca. 1 min"),
]
PLAN_ROWS = "\n".join(
  f'<div class="plan__row"><span class="plan__n">{i}</span>'
  f'<span class="plan__t"><b>{t}</b><small>{d}</small></span>'
  f'<span class="plan__min">{m}</span></div>'
  for i, (t, d, m) in enumerate(PLAN, 1)
)
...
COPY_SVGS = ('<svg class="i-copy" viewBox="0 0 24 24" aria-hidden="true">'
...
def copyblock(pid, content, cls=''):
    return ('<div class="codewrap"><pre class="cmd ' + cls + '" id="' + pid + '">' + esc(content) + '</pre>'
      ...
      + COPY_SVGS + '</button></div>')

def copyicon_inline(text, aria='Kopiér'):
    return ('<button class="copy-icon inline-copy" data-copy-text="' + esc(text) + '" data-tip="Kopiér" '
      'aria-label="' + esc(aria) + '" type="button">' + COPY_SVGS + '</button>')
...
<style>
:root{{
  --bg:#f7f5f1; --surface:#ffffff; --ink:#13161a; --muted:#65696f;
  --line:#ced1d4; --band:#eeebe5; --accent:#ca8a10; --accent-soft:#f9e1c0; --accent-ink:#a84a00;
  --btn-flade:#ca8a10; --btn-tekst:#13161a;
...
@media (prefers-color-scheme:dark){{
  :root{{ --bg:#0b121a; --surface:#151b21; --ink:#f7f5f1; --muted:#a0a5ab;
    --line:#36383a; --band:#151b21; --accent:#ca8a10; --accent-soft:#3a2c17; --accent-ink:#e8a93a;
    --btn-flade:#ca8a10; --btn-tekst:#13161a;
    --stab:#6fb3a6; --stab-soft:#1e2e2a; --afd-line:#3a3227; --code-bg:#0f0c08; --code-ink:#e9dcc6; }}
}}
:root[data-theme=dark]{{ --bg:#0b121a; --surface:#151b21; --ink:#f7f5f1; --muted:#a0a5ab;
  --line:#36383a; --band:#151b21; --accent:#ca8a10; --accent-soft:#3a2c17; --accent-ink:#e8a93a;
  --btn-flade:#ca8a10; --btn-tekst:#13161a;
  --stab:#6fb3a6; --stab-soft:#1e2e2a; --afd-line:#3a3227; --code-bg:#0f0c08; --code-ink:#e9dcc6; }}
:root[data-theme=light]{{ --bg:#f7f5f1; --surface:#ffffff; --ink:#13161a; --muted:#65696f;
  --line:#ced1d4; --band:#eeebe5; --accent:#ca8a10; --accent-soft:#f9e1c0; --accent-ink:#a84a00;
  --btn-flade:#ca8a10; --btn-tekst:#13161a;
  --stab:#3f6b63; --stab-soft:#e0ece9; --afd-line:#d8cfc0; --code-bg:#2b2117; --code-ink:#f3e7d4; }}
...
.navcta{{flex:none;background:var(--btn-flade);color:var(--btn-tekst);text-decoration:none;font-size:13.5px;
...
.btn--primary{{background:var(--btn-flade);border-color:var(--btn-flade);color:var(--btn-tekst)}}
...
/* install */
.install{{margin:12px 0 8px;scroll-margin-top:76px}}
...
.plan__row{{display:grid;grid-template-columns:24px 1fr auto;gap:6px 12px;align-items:baseline;
  padding:12px 0;border-bottom:1px solid var(--line)}}
.plan__n{{font-weight:700;color:var(--accent-ink);font-variant-numeric:tabular-nums}}
.plan__t{{font-size:14.5px;min-width:0}}
.plan__t small{{display:block;color:var(--muted);font-size:13px;margin-top:3px;line-height:1.5}}
.plan__min{{font-size:13px;color:var(--muted);white-space:nowrap;font-variant-numeric:tabular-nums;
  background:var(--band);border:1px solid var(--line);border-radius:999px;padding:3px 10px}}
.plan__sum{{display:flex;gap:14px;align-items:baseline;justify-content:space-between;flex-wrap:wrap;
  margin:14px 0 0;color:var(--muted);font-size:13.5px}}
.plan__sum b{{color:var(--ink);white-space:nowrap}}
@media (max-width:560px){{.plan__row{{grid-template-columns:24px 1fr}}.plan__min{{grid-column:2;justify-self:start}}}}
...
.copy-icon::after{{content:"";position:absolute;inset:-8px}} /* 46px hit-area, visuel stoerrelse uaendret */
...
.copy-icon.inline-copy{{position:static;width:24px;height:24px;vertical-align:middle;margin-left:8px;border-radius:6px}}
.copy-icon.inline-copy svg{{width:13px;height:13px}}
...
.nyband{{background:var(--band);border-top:1px solid var(--line);border-bottom:1px solid var(--line);
  margin:52px 0 8px;padding:clamp(38px,5vw,62px) 0}}
.nb__grid{{display:grid;grid-template-columns:1.15fr 1fr;gap:clamp(28px,4vw,64px);align-items:end}}
.nb__eyebrow{{font-size:12.5px;letter-spacing:.15em;text-transform:uppercase;color:var(--accent-ink);
  font-weight:600;margin:0 0 12px}}
.nb__head h2{{font-family:var(--serif);font-size:clamp(28px,3.4vw,46px);line-height:1.06;
  letter-spacing:-.018em;margin:0;font-weight:600}}
.nb__lead{{margin:16px 0 0;color:var(--muted);font-size:15.5px;line-height:1.65;max-width:46ch}}
.nb__form{{display:grid;grid-template-columns:1fr auto;gap:8px;border-radius:999px;
  background:var(--surface);border:1px solid var(--line);padding:6px}}
.nb__form:focus-within{{border-color:var(--accent-ink);box-shadow:0 0 0 3px var(--accent-soft)}}
.nb__form input[type=email]{{border:none;background:transparent;color:var(--ink);padding:13px 16px;
  font-size:15px;font-family:inherit;outline:none;min-width:0}}
.nb__form button{{background:var(--ink);color:var(--bg);border:none;border-radius:999px;
...
@media (max-width:880px){{
  .nb__grid{{grid-template-columns:1fr;align-items:start}}
}}
...
@media (max-width:560px){{
  ...
  /* Inline grid-template (1.05fr 1fr) paa Trin 2/3 ville ellers klemme kortene
     til ~170px-spalter side om side paa mobil. */
...
<header class="hero"><div class="wrap">
  ...
  <div class="btnrow">
    ...
    <a class="btn" href="#opdater">Opdater tidligere installation</a>
    <a class="btn" href="#plan">Se de fem trin (ca. 1 time)</a>
  </div>
</div></header>
...
<main class="wrap">
  <section class="prose" id="intro">
    ...
    <p><strong>Du har allerede adgang til en AI, der kan skrive dine tilbud, læse dine filer og huske din virksomhed.</strong> Claude er på plads i tusindvis af danske virksomheder, og modellen bliver bedre for hver måned. Teknologien er ikke længere flaskehalsen.</p>
    <p><strong>Men de fleste bruger den som en klogere søgeboks - og det er problemet.</strong> Uden fast viden svarer AI'en i blinde, og du forklarer virksomheden forfra i hver eneste samtale. Uden faste arbejdsgange bliver hver opgave et nyt eksperiment med nyt resultat. Og det, I lærer sammen, fordamper, når chatten lukker. En chatbot, der svarer, er ikke en medarbejder, der arbejder - forskellen er ikke modellen, det er opsætningen.</p>
    <p><strong>Denne side ER opsætningen: en komplet AI-organisation.</strong> {total} færdige AI-medarbejdere (skills) i et klassisk dansk organisationsdiagram - hver skill er én afgrænset, tilbagevendende opgave (tilbuddet, referatet, rykkeren, budgetopfølgningen, illustrationen) med sin faste arbejdsgang, sine gates (intet sendes uden din godkendelse) og sine aldrig-regler. Du skal ikke huske kommandoer: skriv "lav et tilbud til Nordkøb", og den rette medarbejder melder sig. Alt er bygget til danske SMV'er og valideret mod rigtige jobfunktioner, ikke oversat fra amerikanske skabeloner.</p>
    <p>Under skillsene ligger hukommelsen: <strong>company brain'en</strong> - en helt almindelig mappe med markdown-filer (rene tekstfiler, du selv kan åbne og læse) på din egen maskine. Ingen database, ingen leverandør-silo; bare filer du kan åbne, flytte og tage backup af. Samarbejdet mellem skills og hjerne går gennem tre hub-filer i hjernens identity/-mappe: <code>virksomhedsprofil.md</code> (hvad I sælger, jeres priser, hvem I siger nej til), <code>voice-profil.md</code> (hvordan I lyder, dokumenteret med citater fra jeres egne tekster) og <code>designprofil.md</code> (hvordan I ser ud). Alle {total} skills læser dem, før de arbejder - derfor rammer tilbuddet jeres prisliste og opslaget jeres tone, uden at du forklarer det forfra.</p>
    <p>Og viden samles op i stedet for at fordampe. Undervejs i arbejdet skriver Claude selv nyt ind i hjernen som daterede noter med kilde, og varige beslutninger gemmes i <code>beslutninger.md</code> MED begrundelsen - de står ved magt, til du udtrykkeligt ændrer dem, så intet diskuteres forfra. Når du starter en ny samtale, læser Claude hjernens indeks, FØR den svarer: i Claude Desktop/Cowork via en fast åbningsregel, prompten har skrevet ind (virker, så snart du arbejder i hjernemappen), og i Claude Code automatisk i alle sessioner via en indbygget hook. Og læringen kræver ingenting af dig: ved <strong>dagsskiftet</strong> - dagens første samtale - samler Claude selv op fra i går (automatiske dagsspor og den natlige høst lander i hjernens indbakke, og morgenopsamlingen skriver det væsentlige ind) og kvitterer kort med, hvad hjernen har lært. Vil du gemme med det samme, kan du stadig skrive <em>"luk sessionen"</em> - men skriver du intet, tager dagsskiftet den i morgen. Organisationen bliver klogere for hver dag.</p>
    <p><strong>Næste skridt er installationen - og rækkefølgen er hele pointen. Fem trin, ét ad gangen:</strong> 1) vælg hjernens mappe · 2) byg hjernen · 3) installér skills · 4) udfyld hub-filerne · 5) tjek det hele. Sæt cirka en time af i alt; overblikket med tidsforbrug står lige nedenfor, og hvert trin er forklaret bagefter i præcis den rækkefølge.</p>
  </section>

<section class="nyband" aria-labelledby="nyhedsbrev"><div class="wrap">
  <div class="nb__grid">
    <div class="nb__head">
      <p class="nb__eyebrow">Nyhedsbrev · lav frekvens, høj substans</p>
      <h2 id="nyhedsbrev">Tilmeld dig nyhedsbrevet og få en detaljeret guide til, hvordan dit AI-hold virker i praksis.</h2>
      <p class="nb__lead">Guiden lander i din indbakke med det samme: de seks lag, dine 76 medarbejdere og ugerytmen, der holder holdet i gang. Derefter nyhedsbrevet hver anden mandag. Afmeld med ét klik.</p>
    </div>
    <div class="nb__side">
      <form id="nbform" class="nb__form" novalidate>
        <input id="nbmail" type="email" name="email" required autocomplete="email"
               placeholder="din@mail.dk" aria-label="Din e-mailadresse">
        <input type="text" name="botcheck" tabindex="-1" autocomplete="off" aria-hidden="true" class="nb__hp">
  ...
  <section class="install" id="plan">
    <p class="section-label" style="margin-top:34px">Overblik - de fem trin og hvad de tager</p>
    <div class="icard">
      <p class="sub" style="margin:0 0 14px">Ca. 45-60 minutter i alt. Du kan holde pause mellem trinene - intet går tabt undervejs. Trin 2 og 4 er samtaler med Claude, hvor du bare svarer på spørgsmålene; trin 1, 3 og 5 er hurtige og tekniske.</p>
      <div class="plan">
{PLAN_ROWS}
      </div>
      <p class="plan__sum"><span>Ingen GitHub-konto, intet login og ingen git undervejs - installeren henter alt over almindelig https fra {DOMAIN}.</span> <b>I alt ca. 45-60 min</b></p>
    </div>
  </section>

  <section class="install" id="trin1">
    <p class="section-label" style="margin-top:34px">Trin 1 - vælg hjernens mappe</p>
    <div class="icard">
      <p class="sub" style="margin:0">Opret en helt almindelig, synlig mappe på din computer, hvor hjernen skal bo - fx <span class="inline">Dokumenter/company-brain</span>. Det tager 10 sekunder, og det er ALT, hvad trin 1 er. Én regel: vælg selv mappen, og lad aldrig appen placere den for dig - ellers ender hjernen gemt dybt i appens systemmapper, hvor du ikke kan finde, flytte eller tage backup af den.</p>
    </div>
  </section>

  <section class="install" id="brain">
    <p class="section-label" style="margin-top:34px">Trin 2 - byg din company brain</p>
    <div class="install__cols" style="grid-template-columns:1.05fr 1fr">
      <div class="icard">
        ...
        <div class="step"><b>a</b><div>Åbn en ny <span class="inline">Cowork</span>-samtale i Claude Desktop, og giv den adgang til mappen fra trin 1 (via mappevælgeren).</div></div>
  ...
  <section class="install" id="installer">
    <p class="section-label" style="margin-top:34px">Trin 3 - installér skills-pakken (én kommando, Claude Code)</p>
    <div class="install__cols" style="grid-template-columns:1.05fr 1fr">
      <div class="icard">
        ...
        <div class="step"><b>0</b><div><b>Ingen konto, intet login, ingen git.</b> Installeren henter hele pakken som ét arkiv over almindelig https fra {DOMAIN}. Du skal hverken have en GitHub-konto, git installeret eller godkende adgang til noget - og bliver du bedt om at logge ind undervejs, er det gået galt: så er der peget på en GitHub-URL i stedet for på installeren.</div></div>
        ...
        <div class="step"><b>2</b><div><b>Idempotent opgradering.</b> Installeren lægger sig selv i <span class="inline">~/.claude/din-ai-org/install.sh</span>. Derfra er opdatering ét kald: <span class="inline">~/.claude/din-ai-org/install.sh update</span> henter nyeste version, fjerner udgåede skills og rører intet af dit eget.</div></div>
        ...
        <div class="step"><b>4</b><div><b>Ren afinstallation.</b> <span class="inline">~/.claude/din-ai-org/install.sh uninstall</span> fjerner kun det, installeren lagde, aldrig dine egne filer.</div></div>
      ...
      <div class="icard">
        ...
        <p class="sub" style="margin-top:8px">Nemmest: indsæt prompten i Claude Code (lige nedenfor) og lad den klare det hele. Vil du selv i terminalen, er det denne ene linje:</p>
        {copyblock("oneline", GIT_BLOCK)}
        <p class="sub" style="margin-top:14px">Eller trin for trin, hvis du vil se planen først:</p>
        {copyblock("installerblock", INSTALLER_BLOCK)}
        <p class="sub" style="margin-top:10px"><span class="inline">plan</span> installerer intet - den viser bare de fem trin med tidsforbrug. Bruger du <b>Claude Desktop</b>? Installeren er til Claude Code - brug zip-upload i stedet (alternativet lige nedenfor).</p>
      </div>
    </div>
  </section>

  <section class="install" id="install">
    <p class="section-label" style="margin-top:34px">Trin 3, sådan siger du det til Claude - og alternativet til Claude Desktop</p>
    <div class="install__cols">
      <div class="icard">
        <h3>Claude Code</h3>
        <p class="sub">Indsæt prompten herunder. Claude viser dig først de fem trin med tidsforbrug, spørger om du er klar, og installerer så. Prompten nævner med vilje ikke GitHub - det er dét, der ellers får Claude til at bede om et login, den slet ikke har brug for.</p>
        ...
        <p class="sub" style="margin-top:14px">Bliver du på noget tidspunkt bedt om at logge ind på GitHub, så stop: sig til Claude, at den skal bruge kommandoen fra {DOMAIN} i stedet. Installationen kræver ingen konto.</p>
      </div>
      <div class="icard">
        <h3>Claude Desktop</h3>
        <p class="sub">Upload én skill-mappe ad gangen som .zip.</p>
        <div class="step"><b>1</b><div>Hent afdelingens zip nederst på denne side - <span class="inline">⬇ Zip</span>-knappen ved hver afdeling. Ingen konto, intet login.</div></div>
  ...
  <section class="install" id="trin4">
    <p class="section-label" style="margin-top:34px">Trin 4 - udfyld hub-filerne (så alle skills kender jer)</p>
    <div class="icard">
      <p class="sub" style="margin:0 0 10px">Kør de tre skills, der skriver hub-filerne - bare sig sætningerne til Claude, én ad gangen:</p>
      <div class="step"><b>a</b><div>"Opret vores virksomhedsprofil" → skillen <span class="inline">virksomhedsprofil</span> henter jeres stamdata og interviewer dig om resten.</div></div>
      <div class="step"><b>b</b><div>"Lav vores toneprofil" → skillen <span class="inline">toneprofil</span> analyserer jeres egne tekster og fastholder, hvordan I lyder.</div></div>
      <div class="step"><b>c</b><div>"Fastlæg vores designretning" → skillen <span class="inline">designretning</span> vælger stil, farver og typografi sammen med dig.</div></div>
      <p class="sub" style="margin-top:10px">Fra nu af rammer tilbuddet jeres priser, opslaget jeres tone og designet jeres retning - uden at du forklarer noget forfra.</p>
    </div>
  </section>

  <section class="install" id="trin5">
    <p class="section-label" style="margin-top:34px">Trin 5 - tjek at det hele hænger sammen</p>
    <div class="icard">
      <p class="sub" style="margin:0 0 10px"><b>Claude Code:</b> kør sundhedstjekket - det viser med [OK] og [MANGLER], om hooks, hjerne og hub-filer står rigtigt, og hvad du gør ved hver linje, der mangler:</p>
      {copyblock("statusblock", "./install.sh status")}
      <p class="sub" style="margin-top:10px"><b>Kun Claude Desktop?</b> Spørg i stedet Claude: "hvor ligger hjernen?" - den skal svare med mappen fra trin 1 og bekræfte, at hub-filerne findes. Gør den det, er du i mål: organisationen arbejder, og hjernen lærer selv af hver dag.</p>
    </div>
  </section>
  ...
  <section class="install" id="opdater">
    ...
    <div class="install__cols">
      <div class="icard">
        ...
        <p class="sub" style="margin-top:14px">Eller direkte i terminalen - installeren ligger allerede på maskinen:</p>
        {copyblock("updatecmd", LOCAL_CMD + " update")}
...
<footer><div class="wrap">
  ...
  Installationen kræver ingen GitHub-konto. Vil du kigge i koden: <a href="{REPO}" target="_blank" rel="noopener">{REPO.replace('https://','')}</a> · <a href="{REPO}/blob/main/README.md" target="_blank" rel="noopener">README &amp; installationsguide</a></p>
</div></footer>

<script>
(function(){{
  var f=document.getElementById('nbform'); if(!f) return;
  var svar=document.getElementById('nbsvar'), felt=document.getElementById('nbmail');
  var RE=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;
...
(function(){{
  ...
  document.querySelectorAll('.copy-icon').forEach(function(cb){{
    cb.addEventListener('click',function(){{
      var t=cb.dataset.copyText, el=null;
      if(t===undefined){{el=document.getElementById(cb.dataset.copy); if(!el)return; t=el.textContent;}}
      ...
      else if(el){{var r=document.createRange();r.selectNode(el);var s=getSelection();s.removeAllRanges();s.addRange(r);
