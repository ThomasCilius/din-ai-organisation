...
description: Systematically analyze an unfamiliar codebase and generate a structured onboarding guide with architecture map, key entry points, conventions, dependencies, risk areas, and a starter CLAUDE.md. Use when taking over a client's codebase, joining a new project, or setting up Claude Code for the first time in a repo.
origin: ECC
---

# Codebase Onboarding

Systematically analyze an unfamiliar codebase and produce a structured onboarding guide. Designed for the consultant scenario: you are handed a client's repository, often with the original developers gone or unavailable, and you need a trustworthy map before you can quote, estimate, or safely change anything. Equally useful for developers joining a new team or setting up Claude Code in an existing repo for the first time.

## When to Use

- Taking over a customer's or client's codebase as an external consultant
- First time opening a project with Claude Code
- Joining a new team or repository
- User asks "help me understand this codebase"
- User asks to generate a CLAUDE.md for a project
- User says "onboard me" or "walk me through this repo"
- Due diligence before quoting on maintenance or feature work
...
**Dependencies**
- Which external services does the code call? (payment providers, email, auth, third-party APIs)
- Which dependencies are pinned, outdated, or abandoned?
- Are there lockfiles, and are they tracked by git?
...
This is where the consultant earns their fee. Before touching anything, map where the codebase can hurt you or the client:

**Change Risk**
- Areas with no test coverage: which directories have logic but no matching tests?
- God files: files far larger than the project's norm, touched by many commits
- Hot spots: files with the highest change frequency in git history (`git log --format= --name-only | sort | uniq -c | sort -rn`), these are where bugs cluster
- Dead or duplicated code that suggests abandoned refactors

**Operational Risk**
- Secrets handling: `.env` files tracked by git, hardcoded credentials, keys in config
- Deployment: is there a documented deploy path, or does deployment live in someone's head?
- Missing or broken CI: do the existing tests even run?
- Environment drift: does `.env.example` match what the code actually reads?

**Knowledge Risk**
- Undocumented business rules buried in code (magic numbers, hardcoded special cases)
- Dependencies on external systems nobody mentioned (cron jobs, webhooks, FTP drops)
- TODO/FIXME/HACK comments that hint at known-but-unfixed problems

Rate each identified risk HIGH / MEDIUM / LOW and note it in the onboarding guide. For a client handover, this section doubles as the honest state-of-the-codebase report.

### Phase 5: Generate Onboarding Artifacts
...
[Key libraries that shape the code, external services called, dependency health]

## Risk Areas
| Risk | Severity | Location | Note |
|------|----------|----------|------|
| [e.g. no tests around billing logic] | HIGH | src/billing/ | [what could go wrong] |
...
## Known Risk Areas
- [High-risk areas from the risk assessment: tread carefully here]
```
...
6. **Be honest in the risk section** - a client handover report that hides problems costs trust later. Name the risks, rate them, and keep the tone factual rather than alarmist.
...
- Judging the previous developers in writing - describe the state, not the people

## Examples

### Example 1: Consultant takes over a client's codebase
**User**: "We're taking over maintenance of this repo from the client's previous agency. Map it for me."
**Action**: Run full 5-phase workflow with emphasis on Phase 4 → produce Onboarding Guide with risk table + Starter CLAUDE.md
**Output**: Onboarding Guide that doubles as a state-of-the-codebase handover report, plus a `CLAUDE.md` written to the project root

### Example 2: First time in a new repo
**User**: "Onboard me to this codebase"
**Action**: Run full 5-phase workflow → produce Onboarding Guide + Starter CLAUDE.md
**Output**: Onboarding Guide printed directly to the conversation, plus a `CLAUDE.md` written to the project root

### Example 3: Generate CLAUDE.md for existing project
...
### Example 4: Enhance existing CLAUDE.md
